...
function test_lastoutput_contains {
	...
	grep_cnt=$(grep -c $grepopts "$search" <"$TESTSET_DIR/$TESTSET_LAST_TEST_NR$extension")
	if [ $? -gt 1 ] ; then
		# grep error
...
function test_expect_lastoutput_linecount {
	...
	if [ $? -gt 1 ] ; then
		...
		printf "CHECK %s FAILED. Linecountof test %s is %d (exp=%d)\n" \
			"$TESTSET_LAST_CHECK_NR" "$TESTSET_LAST_TEST_NR" "$linecountact" "$linecountexp"
		printf "========== Output Test %d Begin ==========\n" "$TESTSET_LAST_TEST_NR"
		cat "$TESTSET_DIR/$TESTSET_LAST_TEST_NR$extension"
		printf "========== Output Test %d End ==========\n" "$TESTSET_LAST_TEST_NR"
		TESTSET_TESTFAILED="$TESTSET_TESTFAILED $TESTSET_LAST_CHECK_NR"
...
function test_exec_ssh {
	...
	ssh $sshopt "$sshtarget" "$*" >>"$TESTSET_DIR/$TESTSET_LAST_CHECK_NR.out" 2>&1
	testrc=$?
...
function test_exec_kubecron {
	...
	while [ -z "$testrc" ] ; do
		...
		jobCondition=$(jq -r 'try .conditions[] | select( (.status=="True" ) and ( .type | IN("Complete","Failed") ) ).type' <<<"$jobStatus" 2>&1)
		#shellcheck disable=SC2181 # using $? here helps to keep the structure
		if [ "$?" != 0 ] ; then
			printf "%s\nACTIVE=%s\nFAILED=%s\nSUCCEEDED=%s\nCONDSTATUS=%s\n" \
				"$jobStatus" "$jobActive" "$jobFailed" "$jobSucceeded" "$jobCondition" \
				>>"$TESTSET_DIR/$TESTSET_LAST_CHECK_NR.out"
			testrc=3
			break
